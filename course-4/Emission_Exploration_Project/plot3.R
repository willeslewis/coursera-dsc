...
PM25tots<-NEI[NEI$fips=="24510",]
PM25sum<-ddply(PM25tots,.(year,type),summarize,total=sum(Emissions))

qplot(year,total,data=PM25sum,facets=.~type,main="Total emissions in Baltimore by type",ylab="Total in tons")
ggsave("plot3.png")